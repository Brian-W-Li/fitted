...
- `plans/m4-data-model-migration.md` - **completed M4 reference** (C1–C8 ✅ done; §14 is the ladder, §14.5 the M5-handoff). **Next active work: the H26 compatibility spike (in build — C1–C4 code committed, B2 `selection.json` sealed; now the C4 RUN phase (panel calibration → calibration pilot → blind addendum freeze → gate-b → emit); `plans/h26-compatibility-spike-v2.md`), then the M5 cutover** — sequence consolidation → H26 → M5 (`Fitted_Spec_v2.md` §20). M5-forward readiness gaps live in `plans/post-m4-readiness.md` (its H26 §2 is superseded by the v2 H26 build doc; only its M5 sections stay active).
