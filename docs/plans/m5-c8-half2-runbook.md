...
- **F5 / History curation (Brian-specified design)** — a user must be able to **curate their History** without deleting the whole account: (a) **flip** a like↔dislike, (b) **remove** a reaction, (c) **delete** feedback entries entirely (the "little bro added 5 reactions" case — they should not just be reversed but *cease to exist*). Reversal must NOT require re-generating that exact outfit (impossible). Two write modes: **flip/remove** = append a new event (the append-only path; latest-state reflects it); **delete** = a genuine hard-delete of the user's own rows (consistent with account-delete — the user owns their data). History reflects all three immediately. NOTE this partially relaxes the strict server-side append-only invariant for USER-INITIATED curation (not the write path) — deliberate, promise-driven.
- **NEW-C** — History shows the RAW append-only feedback log (`getInteractions`, `lib/interactions.ts:371` — no collapse), so a one-tap dislike + its "tell us why" submission appear as **two** entries for one outfit. NOT a data/training bug (the reducer collapses to H61 latest-state per `{snapshotId,candidateId}`), only a display duplication. Fix: dedup the History query by `{snapshotId,candidateId}` (latest per outfit), same as the reducer — folds naturally into the F5 curation work.
- **NEW-D (perf, real)** — History can take up to *minutes* to load on a cold hit. **Root cause is infra, not the query** (verified: all indexes built on M0, the query runs ~180ms fully index-covered, client fires 2 parallel fetches once). It's the **free-tier Atlas M0 cold-connection latency** (sole user → Vercel functions always cold → each load pays a fresh M0 connect). Applied one code lever (`autoIndex:false` in prod — was re-syncing all indexes on every cold instance). Residual cure is infra: warm the connection / a paid Atlas tier (M2 ~$9/mo has vastly faster connects) — a cost/scope call for the study.
