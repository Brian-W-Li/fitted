...
- **Two-stage caching** (R1): **(SUPERSEDED at M5 — plan D2, 2026-07-06: the candidate cache is removed
  entirely; regenerate = one constrained fresh generation with lineage. This bullet and §6.7 are rewritten
  when M5 C4/C5 land; until then `docs/plans/m5-cutover.md` §C wins on conflict.)** the cache stores the **expensive upstream stage** (sampled pool + GPT
...
- `wardrobe/[id]/image/route.ts`: retained wardrobe image upload now has a cheap
  `Content-Length` precheck plus the storage-layer 5MB cap, but an absent/lying header still reaches
  `request.formData()`; true request-size/streaming enforcement remains a W-track host hardening item.
...
hardening (the `interactions` ownership gate is the §16 feedback-authenticity gate). The auth/ownership gates
remain open; the wardrobe image upload has only the cheap `Content-Length` mitigation above, not a true
request-size/streaming boundary.

**Client-side state gates (verified 2026-07-06, independent-audit reconcile — the API register above is
backend-only; these are the legacy *frontend*, also M5-owned).** The recommend/wardrobe UI carries state
bugs the API gates don't touch:
- **Cross-account leak in one browser tab:** the dashboard persists recommendation state under a **global**
  `sessionStorage` key `fitted_dashboard_state` (`dashboard/page.tsx:52`), restored for *any* signed-in user
  on mount (`:637`, no per-uid key) and **not cleared on logout** (`:666`) — user B on the same tab sees
  user A's event text + shown outfits. Fix at M5: namespace the key by uid + clear on sign-out.
- **Redirect-before-sync:** `RedirectIfAuthenticated` (`(app)/RedirectIfAuthenticated.tsx:20`) redirects a
  Firebase-authed user to the dashboard before the sign-in page runs the Mongo `auth/sync` (`signin/page.tsx:43`),
  so a first-time user can land before their User row exists.
- **Silent save-loss:** `AddItemModal` always closes after `onSave` (`wardrobe/page.tsx:403`) while parent
  handlers `setError(...); return` instead of throwing (`:1342`) — a failed wardrobe save closes the modal and
  discards the user's entered item.
- **Residual update/id validation:** the retained wardrobe POST/PATCH routes now reject malformed scalar/array/boolean
  request bodies before persistence (2026-07-06 hardening), so the old `"false"`→`true` and scalar→`[]`
  coercions are closed. The route still relies on raw dynamic params reaching Mongoose, so malformed ObjectIds
  can produce `CastError`→500 instead of a stable 400; M5 validates ids and request envelopes through the
  §15 request adapter (R12).
- **Minor stale/racy state (report-only):** geolocation omitted from a callback dep (`dashboard/page.tsx:726`);
  CV analysis can clear loading for the wrong request (`wardrobe/page.tsx:1490`); index-based feedback rollback
  (`dashboard/page.tsx:754`). Fold into the M5 UI rewrite.

The remaining items here are legacy-frontend / retained-route hardening owned by the M5 cutover (deletion
license: `fitted/` code is rewritten, not patched piecemeal). Registered here so the M5 UI/API rewrite closes
them; the malformed wardrobe body coercions called out by the 2026-07-06 audit are already actioned.
...
**Sequence from here (set 2026-06-27, post-M4 consolidation).** The order is **consolidation → H26 spike ✅ (done 2026-07-05; NO-GO by the frozen letter — see the §20 H26 row + `ml-system/experiments/h26/results.md`) → M5** (with the small additive **scorer-seam rung** of §23-H28 slotted between H26 and M5 — see the §20 table row; it is a sub-step of the H26→M5 transition, not a milestone, and the H26 verdict makes its *timing* an M5-`/spec` decision: the hook's shape is settled, but with M6 not yet open the rung can land with M5 or be deferred to M6 entry). M5 is now the immediate next rung. The **M5 deletion license is fully open** (no real users; this is a demonstrable-concept fork) — the legacy recommend/regenerate vertical and the §19 dresses string-match arm are **cut, not migrated**, when `USE_ML_SHORTLISTER` flips. Post-M5, the **W-track** (ingestion surface) and **B-track** (text boards → StyleProfile) are **coequal** — neither blocks the other, and the now-explicit someday-launch path (rejoin the CS-148 team *or* post + advertise to friends) makes the B-track user-facing surface a real downstream destination, not a dead end.
...
| H16 | Candidate cache key ⊋ session-seed inputs — retires the v1.2 R1/N1 `cache_key ≡ seed` invariant | **RESOLVED-HERE** | New rule `cache key ⊇ seed inputs`: `intent`/`forcedItemId`/`styleProfileVersion` key the candidate cache (they change GPT candidates) but need not seed the sampler (§15). *(M5 note: the runtime cache is removed — `m5-cutover.md` D2; the rule survives as the input contract of the `candidateCacheKey` field on GenerationSnapshot, which groups identical-input renders.)* |
...
| H53 | **Ingestion must normalize EXIF orientation before embedding.** H26 found all 13 closet phone photos carried EXIF orientation 6 and PIL ignores EXIF on `Image.open` — the first closet probe embedded sideways garments (closet AUC 0.4375 → 0.5625 after `exif_transpose`) (`experiments/h26/results.md` §6/§10, 2026-07-05) | **OPEN** → RESOLVE-IN-W-TRACK/M6-INGESTION | Silent measurement corruption: any embedding built from un-transposed phone photos scores rotated pixels, roughly halving the real compatibility signal — the exact bug that once halved the closet probe. The M6 catalog→closet re-measure (a pre-registered M6 entry condition) rides the W-track (§18) ingestion path, so that path must call `ImageOps.exif_transpose` (or equivalent) before any CV embedding, with a regression fixture on an orientation-6 image. Ties H6/H47 (§18 ingestion). |
| H54 | **GenerationSnapshot has no delete guard.** The immutability contract registers `pre` guards for `updateOne/updateMany/findOneAndUpdate`, `replaceOne/findOneAndReplace`, and `save` (`fitted/models/GenerationSnapshot.ts:481-483`) but **no `pre(["deleteOne","deleteMany"])`** — a raw delete can hard-remove an immutable training snapshot outside the reserved redaction seam (nominated in the 2026-07-02 audit handoff, never promoted) | **OPEN** → CLOSE-WITH-M5-LIVE-WRITE | One-way-door integrity gap: the "immutable training truth" invariant (H10/H29/H43) is enforced against mutation but not deletion. Harmless today (zero-user fork, no delete caller) but M5 adds the live writer, so M5 adds a `pre(["deleteOne","deleteMany"])` rejection — redaction (`redacted:true`) is the only sanctioned removal, never a hard delete — completing the door alongside H43's redaction-cascade wiring. Distinct from H50 (duplicate *write* idempotency). |
| H55 | **`fitted_core` live-generation defaults are pre-H26 / cross-model-inconsistent.** `OpenAIGenerator.__init__` defaults `model="gpt-4o"` + only supports the `max_tokens` param (`ml-system/fitted_core/generation.py:71`/`:101`), while production is `gpt-5.4-mini` (`recommend/route.ts`) and the H26 judge already maps GPT-5.x to `max_completion_tokens` (`experiments/h26/gpt_judge.py:724`) because **GPT-5.x rejects `max_tokens`** (independent-audit find, verified 2026-07-06) | **OPEN** → RESOLVE-AT-M5-WIRING | Latent forward-compat trap, not a live bug: the seam is Generator-injected and tests use fakes, so the stale default is dormant until M5 wires the real generator. `max_tokens` defaults `None` so an uncapped GPT-5.x call still works — the break is only if M5 wires a GPT-5.x model **and** sets a token cap through the `max_tokens`-only path. M5 reconciles the default model to the production stylist (`gpt-5.4-mini`, or an explicit Fable-reviewed choice) and switches the cap param to `max_completion_tokens` for GPT-5.x (mirroring `gpt_judge.py`). **Same class (pre-flight 2026-07-06): the default `temperature=0.8` (`generation.py:72`) diverges from production's `0.5` (`route.ts:455`)** — snapshot provenance is safe either way (temperature is caller-supplied to `build_snapshot_payload`), but M5 must choose the value explicitly, never inherit either default silently. The historical `gpt-4o` default is correct provenance for the Spearhead §E C6 eval — do not retro-edit that record, only the forward default. |
...
| H57 | **The daily-intent engine surface is specced nowhere.** The §20 M5 row's "rewrite of recommend/regenerate routes" implies serving the **daily** intent (the deployed dashboard's main flow) and §19 deletes the legacy vertical at cutover — but `fitted_core`'s only end-to-end orchestrator is the rescue vertical (`rescue()`/`rescue_with_trace`, `rescue.py:646/:849`); `RescueRequest.forced_item_id` is **required** (`rescue.py:87`); `build_snapshot_payload` is `RescueTrace`/`RescueRequest`-typed (`snapshot.py:491`) and hard-codes `intent="rescue_item"` (`snapshot.py:523`) (M5 pre-flight 2026-07-06) | **OPEN** → RESOLVE-IN-M5-`/spec` | The pipeline pieces are intent-generic (sampler → §12 generation → validator → ranker → response), but the composed daily orchestrator + its trace + the snapshot-producer generalization (parameterize `intent`, make the forced item optional) are **new Python surface no doc itemizes**. The M5 `/spec` must scope it explicitly: build the daily orchestrator alongside the cutover, or cut over rescue-first and keep the legacy arm serving daily (which contradicts §19's cutover delete list — that list may not shrink silently). Ties H45 (the rescue delivery route is the inverse gap: engine built, no route feeds it). |
| H58 | **The Next→service contract (API surface, request wire format, auth) has no spec home.** `fitted_core` has no HTTP layer; §15.1/`snapshot_serde` cover only the snapshot-payload direction and §15.2 only item-field renames — the request envelope (endpoints, wire casing for the request/wardrobe, error format) is unspecced, and **service auth is presupposed but never designed** (mentioned only inside H12's trigger list and H13's drift note) (M5 pre-flight 2026-07-06) | **OPEN** → RESOLVE-IN-M5-`/spec` | An unauthenticated always-on Fly.io endpoint that calls OpenAI is an **open LLM-spend proxy** — auth is a spend-safety gate, not polish. The `/spec` authors: the service API (endpoints + request/response JSON incl. casing convention), the auth mechanism (e.g. shared-secret header, service non-public), the error envelope H12's trigger set keys off, **and the §15.2-parallel Lens adapter table** (deployed request → Lens fields: intent routing, the weather raw→bucket threshold rule, constraints sourcing — the readiness §4.1 IOU, previously homeless in this register). Ties H12/H13/H51. |
| H59 | **Regenerate must preflight contradictory locks.** The legacy `regenerate` route can accept the same item in `lockedItemIds` and `dislikedItemIds`, leaving the reroll with an impossible constraint set that can degrade into empty-success / wrong-output behavior instead of a predictable client error (forward-audit reconcile 2026-07-06) | **OPEN** → RESOLVE-IN-M5-ROUTE-REWRITE | M5 deletes the legacy route, so do not patch it now; the rewritten recommend/regenerate contract must reject `locked ∩ disliked ≠ ∅` before candidate filtering (400/409 with a stable error envelope), and tests must pin that no empty successful recommendation response is emitted for contradictory per-request controls. Ties H7 (generationIndex/reroll), H49 (cache-hit reroll provenance), and the §12/R9 regen controls. |
| H60 | **Recommend/regenerate route rewrite must bound user-controlled prompt/spend fields.** The legacy vertical accepts request-body text knobs (`occasion`, style/change notes, feedback/change targets, weather-ish context) and generation options without a centralized length/type clamp or a hard completion-token budget, so a malformed direct API caller can expand prompt size / output spend even though the current fork is not the deployed prod (forward-audit reconcile 2026-07-06) | **OPEN** → RESOLVE-IN-M5-ROUTE-REWRITE | M5's rewritten route/service adapter must validate and clamp all body-controlled text/array fields before they reach the service prompt, set an explicit output-token cap (`max_completion_tokens` for GPT-5.x per H55), and test overlong/ill-typed request bodies plus the cap plumbing. This is distinct from H58 service auth: auth prevents open proxying, while this bounds authenticated or same-user malformed calls. |
