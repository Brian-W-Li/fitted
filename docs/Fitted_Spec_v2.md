...
| H100 | **Regenerate controls (locks / avoids) do not survive a re-roll.** `RegenerateModal` is conditionally mounted (`app/(app)/dashboard/page.tsx:1371`) and seeds `locked`/`disliked` from empty sets (`:345-346`), while the route treats `controls` as regenerate-only and never inherits them from the parent snapshot (`app/api/recommend/route.ts:345`, "controls are regenerate-only; a root render must not carry them"). So every re-roll starts with nothing locked: a friend who locks a piece, re-rolls, then re-rolls again silently loses the lock. Friend-reported (2026-07-27) as "regenerate is not locking the item that was locked in the original prompt". **Record the NON-defect too, so a future session does not "fix" the wrong thing:** the rescue *anchor* is inherited correctly — `route.ts:332` derives `forcedItemId` from the parent, never the client — and this was verified on live data, Marc's only re-roll (`generationIndex=1`, 2026-07-27T03:15) kept the forced athletic short in **all six** child candidates plus the parent's occasion. Sub-note (n=1, do not overclaim): that child's `c0` was item-identical to the parent's `c0` while the F13 copy promises the list was swapped — a watch-item, not yet a finding. | **OPEN — friend-visible, registered not fixed** | Either seed the modal from the parent snapshot's stored `controls` (they are already persisted on the row), or make locks sticky per-outfit for the session. Whichever shape: the server already validates and stores controls, so this is client state only. Do not "fix" the anchor — it is correct. |
