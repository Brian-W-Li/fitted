...
(S4: recent-snapshot read window for the H19 reducer) · `FEEDBACK_DEDUP_WINDOW=300` (seconds; S6:
read-time window collapsing same-`{snapshotId,candidateId,action}` feedback rows in the compute-live
affinity projection — accidental retry counts once, repeat-events outside it each count) ·
`INTERACTION_ROWS_SCAN_LIMIT=500` (bounded interaction-row fetch, §16/plan §H). *(**Provenance trap-guard
