...
| H77 | **Ingestion yield-loss + dead-end on the friend-critical add path (W-track UI).** Two friend-visible defects in `app/(app)/wardrobe/page.tsx`: **(a)** a photo-upload failure during "Save & add another" is invisible — the handler calls the PAGE-level `setError` (banner rendered behind the modal's `fixed inset-0 z-40` overlay) but returns success, so the modal resets and the item saves PHOTO-LESS with nothing on screen; on a batch-add of a 15–30-item closet a rate-limit 429 / >4MB-after-compression throw / flaky network then repeats silently — direct yield loss on the M6-image-critical path. **(b)** the 5MB pick-time cap (`onPickImage`) runs on the ORIGINAL file before `prepareImageForUpload` (the 1280px downscaler built to solve exactly this) ever runs, and the app's own HEIC advice ("set Camera → Most Compatible" → multi-MB JPEGs) can steer a friend into that dead-end with a remedy-free "Max image size is 5MB." (2026-07-24 ingestion audit.) | **OPEN — W-track, friend-facing** | (a) return a distinct "saved-but-photo-failed" signal so the modal surfaces the notice inside itself while still resetting (no dup — the item WAS created); (b) run the downscale at pick, or raise the pick cap to ~10–15MB and let the existing post-compression 4MB gate bound it, or at minimum give the 5MB message the HEIC message's remedy. Fable-review the modal return-contract change; sequence with the W-track ingestion work. Minor siblings (same audit, low/latent): edit hardcodes `size:""/notes:""` clearing those fields on any row that has them (`page.tsx:581-584`, no UI field today); a since-deleted-snapshot history card gives a retry-forever "try again" on Flip (`history/page.tsx:56-62`). |
