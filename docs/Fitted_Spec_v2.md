...
- **No lens component — the known gap (§23-H101).** Both keys are garment-identity only. Because
  they are the edge identity in shipped code, and §11 specs the StyleEdge "under a lens", a 👍/👎
  recorded for one occasion applies to every occasion. Adding a lens to FullSignature would be an
  H30-style append; adding it to BaseKey (what the cooldown uses) is a **redefinition**, not an
  append — which is why H101 is a design call and not a patch.
...
model handles shoes as an optional role honestly. *(**These legacy cites are historical: the whole
pre-M5 vertical was DELETED at M5 C8** — `recommend/route.ts` is now a 32-line dispatcher and
`isValidOutfitStructure` has zero hits repo-wide, so do NOT try to grep the symbol. Read it in git
history if you need the old behavior; §19.)*
...
> **"Under a lens" is the target and is NOT yet true of either field (§23-H101).** §7's canonical
> keys — which are the edge identity in shipped code, and which drive the cooldown and comboBoost —
> carry no lens component, so every behavioural edge collapses across occasions. This sentence stays
> as written on purpose: it is what the graph is *for*, and weakening it to match the keys would
> launder the gap away. H101 owns closing it; decide it together with H28.
...
**Prompt-vs-board precedence** — **the target, NOT current behaviour (§23-H105)**: hard constraints
(dress code / weather / comfort) > prompt occasion & formality > active StyleProfile shapes choices
*within* the valid context > revealed negative signal suppresses bad repeats. A casual board never
overrides a "formal interview" occasion. **Shipped reality:** only the last tier is implemented, and
it is lens-blind (§23-H101) — occasion and weather reach `RankerContext` but are consumed solely as
tie-break RNG seed inputs, `formality` is not in the data model, and StyleProfile exists only as
dormant snapshot columns. Today the LLM prompt is the sole mechanism making an outfit
occasion-appropriate. Read this chain as the destination; do not cite it as a contract.
...
- **Regen controls** (R9): `dislikedItemIds` and `lockedItemIds` are per-request **regen controls**.
  **M5 regenerate = one constrained fresh generation** (no candidate cache, no re-rank — `m5-cutover.md`
  ...
  regen contract:** `changeTarget` and `feedbackNotes` (the pre-M5 `regenerate/route.ts` destructured them;
  locks express the intent, notes persist via the feedback flow). That file **was deleted at M5 C8** —
  `app/api/regenerate/` no longer exists, so the cite is git-history only (§19).
...
- **Feedback-authenticity gate (must precede training)** — confirmed real: `POST /api/interactions`
  (the pre-M5 `interactions/route.ts`, since reduced to a 37-line delegate) authenticated the caller but persisted client-supplied `items` and
...
- **Photo = strong nudge, never a hard block** (D1): the photo is the hero step, with a deliberate,
  honestly-labelled escape hatch that states what a photo-less item costs. The wording is
  **the consequence, not the programme** — since 2026-07-25 the in-app copy says a photo-less item
  shows as a grey box in its outfit ("Save without a photo"), rather than naming a "study" or
  "experiment" the app never explains. (The corpus disclosure lives where it can be consented to —
  the recruit message, runbook §8 — not as an unexplained term of art on a form.) A hard block
  breeds fake-satisfied data (floor photos / abandonment) — the anti-capture failure the ambition
  forbids.
...
- **Taxonomy legibility (H52 rung-2).** Today the UI speaks `category`/`subCategory` (the display/CV
  ...
  knob `layerRole` is already consumed by `deriveClothingType`; only the edit UI is unbuilt). The
  ingestion-time **visibility** slice shipped 2026-07-23: the add/edit form's "Files as" chip derives
...
| **M6 (the dive)** | Trained edge/graph scorer — the **content-compatibility prior lands on the §23-H28 additive pairwise/outfit-level `rank()` hook** (the non-transitive, type-conditioned shape the item-level `SignalScorer` sampler slot cannot represent), **gated by the H26 spike above — which returned NO-GO by the frozen letter (a gate-B power miss at the frozen cap, not an accuracy miss), so M6 does not open on H26's authority alone**: entry requires re-powering gate B (extend the judged prefix over the frozen `fitb_order.json`) **and re-measuring the catalog→closet transfer on real-ingestion data**. **The power condition is now MET** (2026-07-21, `gate_b_extension.py` / §23-H56 / `metrics_extension.json`): at N=1000 the adjudicating gate-B diff is +0.2593 [+0.2260, +0.2987], half-width 0.036327 ≤ δ → powered-pass under the sealed letter (a sequential estimate reported ALONGSIDE — it does not rewrite the frozen N=500 NO-GO). **So the catalog→closet transfer re-measure is the sole remaining M6 entry gate** — H26 *reports* that transfer, it does not gate on the underpowered single closet — build doc §12/§13. **Re-measure prereg FROZEN 2026-07-20 at `ml-system/experiments/track2_transfer/preregistration.md` (+ `.json`)** — the freeze-before-look decision rule (single home; this row summarizes, it does not restate). The inherited H26 healthy band was RETIRED as a gate because it is structurally undecidable at cohort N (re-derived arithmetic, `derive_power.py`: Hanley-McNeil half-widths 0.133/0.115/0.093 at N=30/40/60; the `CI_low(AUC_closet) ≥ 0.70` floor read needs a point estimate *above the catalog ceiling itself* (0.7315) at every reachable N — unpassable; the `CI_high(drop) ≤ 0.12` read is impossible at N≤30 even at a perfect transfer — **this derivation is the trap-guard against a "moved the goalposts" charge: the inherited gate provably could not pass before any label existed**). The frozen rule: **(primary, decision-relevant) accepted-vs-rejected discrimination** within-user, AUC boundary 0.50 (human-judged negatives fix H26 §9.9's mechanical-negative proxy; ~doubles data/friend), decided by a **two-look group-sequential design** (25/arm then 50/arm, 97.5% CIs Bonferroni) with a **point-estimate floor ≥0.60** on ESTABLISHED and a **horizon expiry** (2026-10-31 / service teardown) → UNDERPOWERED-TERMINAL, so it decides even at pessimistic yield; **(secondary, reported-never-gates) two-boundary directional** transfer read {above-chance CI_low>0.50 / below-healthy CI_high<0.70}, the H26-continuous mechanical-negative read — **never promoted to primary post-hoc** even if it reads better; the frozen measurand (H26 trained head + FashionSigLIP + mean-over-edges + EXIF); the `exportTrack2Core.cjs` verdict hardened from a raw ≥30 count to a **scoreable-cluster certificate** (≥2-item outfits, image-usable, same-category-negative availability for transfer, signature-dedup, per-friend concentration cap ≤50%, author-excluded), pinned equal to the prereg by `test_preregistration.py`. Fable-reviewed before freeze (SHIP-WITH-CHANGES, all folded). Offline NDCG@k / AUC / FITB on the universal content prior (zero-user, runnable); the **behavioral/personalization** signal (which *does* ride the item-level `SignalScorer` slot) → learned + online A/B is the **user-dependent arm** (`[STAGED]`/`[NORTH-STAR]` on a no-users fork — §11/§23-H9) | `[STAGED]` |
...
| H14 | Retained-host cleanup bugs: clear-wardrobe/user-cascade omit some cleanup; image **replacement deletes the old image before the replacement commits** (data-loss ordering) | **ALL ARMS CLOSED** (M4b C7 user-cascade + 2026-06-27 clear-route + **image-replace ordering LANDED 2026-07-25**) | M4b C7 extended the `User.ts` cascade (`cascadeDeleteUserData`) to hard-delete `wardrobeimages` (H43's cascade arm, tested in `userCascade.test.ts`), and the **clear-wardrobe route arm is also closed (2026-06-27 readiness): `DELETE /api/wardrobe/clear` now drops `wardrobeimages` too via the extracted `lib/clearWardrobe.ts` (`wardrobeClear.test.ts`)** — so both cleanup omissions in the hole text are fixed. **(D2/REPLACE-1, Track 2 2026-07-17: item-delete AND clear-wardrobe now KEEP a snapshot-referenced image — `$nin: keepIds` via `lib/imageReferences` — for M6 image-embedding provenance; account-delete still purges ALL, so erasure holds. So clear drops only UNREFERENCED images, not all.)** The GenerationSnapshot redaction seam was **reserved, not wired** in M4; the snapshot delete path landed at Track 2 as **erasure** (§23-H43 RESOLVED), so the `User` cascade now hard-deletes `generationsnapshots` too. **The image-replacement ordering arm is CLOSED, not W-track deferred: `app/api/wardrobe/[id]/image/route.ts` is now store → repoint → delete (`:163-191`), so every failure arm leaves the old photo intact and `imagePath` never names a deleted row.** The load-bearing throw is reachable, not theoretical — a HEIC renamed `.jpg` reports `type: "image/jpeg"`, passes the client allowlist, survives the client downscale (which cannot decode it, so it forwards the ORIGINAL bytes) and then fails the server's magic-byte sniff — and the §23-H77(a) notice makes "retry the photo from Edit" the ADVERTISED remedy, routing friends through exactly this path. **Trap-guard: the ORDER is the guarantee — do not reintroduce delete-before-store "to keep the byte budget exact".** The transient overshoot is one image (≤`MAX_WARDROBE_IMAGE_BYTES`, 5MB) for the duration of one `deleteOne`, immaterial against [[H67]]'s ~107MB-per-at-cap-account arithmetic. Regression-pinned in `tests/wardrobeImageUpload.test.ts` over BOTH reachable throw arms (sniff-reject 415, storage-cap 413), each asserting the old row, the pointer, and the absence of an orphan; mutation-proven — and note those pins were added only on 2026-07-25 (audit), so before them the revert left all 17 tests green |
...
| H54 | **GenerationSnapshot has no delete guard.** The immutability contract registers `pre` guards for `updateOne/updateMany/findOneAndUpdate`, `replaceOne/findOneAndReplace`, and `save` (`fitted/models/GenerationSnapshot.ts:606-608`) but **no `pre(["deleteOne","deleteMany"])`** — a raw delete can hard-remove an immutable training snapshot outside the reserved redaction seam (nominated in the 2026-07-02 audit handoff, never promoted) | **RESOLVED — delete guard LANDED (C5/C6)** | One-way-door integrity gap: the "immutable training truth" invariant (H10/H29/H43) is enforced against mutation but not deletion. Harmless today (zero-user fork, no delete caller) but M5 adds the live writer, so M5 adds a `pre(["deleteOne","deleteMany"])` rejection **at the Mongoose layer**. Redaction (`redacted:true`) is the only sanctioned *in-place* mutation; the sole sanctioned **hard-delete** is account-deletion **erasure via the `User` cascade** (native driver, deliberately below this guard — §23-H43 Track 2 policy), so the guard blocks stray deletes without blocking the one legitimate erase. **Residual (test-lane):** the Mongoose guard does not cover *native-driver* deletes (the cascade's own layer), and the source-grep static test that m5-cutover.md's acceptance list envisioned for unsanctioned raw `generationsnapshots` deletes was never built — pre-existing, low-severity (the sole native-driver **`generationsnapshots`** delete caller is the sanctioned cascade — the Track-2 feedback-curation `DELETE /api/interactions` and the POST erasure self-heal are native-driver deletes on **`outfitinteractions`** only, never snapshots, so this guard's scope is unaffected). Distinct from H50 (duplicate *write* idempotency). |
...
| H56 | **Gate-B repower tooling must extend, not delete, the frozen judged prefix.** `run_judge.py`'s `_guard_gate_b_ledger` (`:201`) deletes a committed-clean `judge_runs.ndjson` before regenerating — a delete-then-rebuild that fought the M6 repower path (`results.md` §10: extend the frozen-ordered judged prefix past N=500 to close the +3.02e-4 power miss) (independent-audit find, verified 2026-07-06) | **IMPLEMENTED (2026-07-21)** — `ml-system/experiments/h26/gate_b_extension.py` (Fable-reviewed SHIP-WITH-CHANGES, all folded) | Repower is a **SEPARATE additive tool**, `gate_b_extension.py` (`freeze`/`run`/`analyze`): it judges questions `[500:N_ext]` of the SAME C3-frozen order (a longer prefix, never a re-selection) into its OWN `judge_runs_extension.ndjson` (append-only, `group_samples` keep-last dedup → resume-safe), and `analyze` CONCATENATES the two ledgers on read. **Trap-guard:** it does NOT append into `judge_runs.ndjson` — that file stays byte-identical so its `metrics.json._meta.judge_ledger_sha256` bind keeps holding and the frozen N=500 verdict is never rewritten (concatenation-on-read is semantically identical to an append but preserves the sealed bytes; `cmd_freeze` asserts the original ledger's sha == `metrics.json`'s before freezing). `_guard_gate_b_ledger`'s delete is now scoped-and-documented (`run_judge.py:184`) to the original frozen-500 *reproduction* only (capped at `gate_b_cap`=500); the repower path never calls it. **Both hardening notes discharged (sealed prereg NOT edited):** (1) `cmd_analyze` INHERITS the sealed letter (`verdict = A∧B∧D`, `underpowered_is_no_go: true`) and STATES — not re-litigates — the vacuity corner: if `judge_above_chance` is false at N_ext the B-read is vacuous and the underpowered corner still adjudicates NO-GO (`_README` + print). (2) `assert_choice_ranges` fails loud on any cached row whose non-null `choice` is out of `[0, n_candidates)` (which `collapse_question`'s reverse remap would otherwise turn into a silent miss), called in `cmd_run` before any spend AND in `cmd_analyze` before scoring; pinned by `tests/test_gate_b_extension.py`, whose mirror-equality test also pins `extension_letter` against the real `apply_gates` `b_leg` + the frozen `metrics.json`. Run COMPLETE (2026-07-21, `metrics_extension.json`): at N_ext=1000 (991 kept / 9 dropped) the adjudicating gate-B diff is +0.2593 [+0.2260, +0.2987], **half-width 0.036327 ≤ δ → powered-pass** (both conventions agree; judge non-vacuous, CI_low 0.327) — the projected 0.0356 held. The frozen N=500 NO-GO is untouched (a sequential estimate reported alongside). This closes the H26 NO-GO's power lever (§20 M6 row); the catalog→closet transfer re-measure remains the sole open M6 entry gate. |
...
| H66 | **Geo weather discarded temperature.** With a location present, `resolveWeatherProd` fetched a rich summary (e.g. "Clear sky, 34°C") then bucketed it via `bucketFromSummary`, a condition-TEXT keyword matcher — the WMO summaries contain no hot/mild/cold keyword except "snow", so every non-snow temperature collapsed to "mild". The bucket is the only temp signal reaching the stylist prompt and the `WEATHER_WARMTH_BAND` option-path display bucket (`fitted_core/response.py:208-209` — **not** a ranker penalty; the ranker reads `context.weather` only as a tie-break RNG seed input, §23-H105), so a 34°C day was styled "mild" and the M6 corpus recorded `weather:"mild"` for hot/cold renders. | **RESOLVED — numeric bucketing LANDED** (`lib/weather.ts` + `lib/mlRecommend.ts` `bucketFromTemp`) | `getWeatherContext` now returns `tempC`/`feelsLikeC` (center-slot for the forecast path); the geo path buckets by the NUMBER, feels-like preferred: **cold ≤ 10°C, hot ≥ 24°C, mild between** (aligned to the warmth bands), with a **snow-in-summary → cold** override. `bucketFromSummary` stays the no-geo occasion-text fallback. Fulfills the m5-cutover §F "temp/condition → bucket" mandate (was half-implemented). Thresholds AND the `resolveWeatherProd` geo→`bucketFromTemp` wiring are pinned in `contextDetection.test.ts` (the wiring test is mutation-proven: reverting `lib/mlRecommend.ts:158` to `bucketFromSummary` reddens it). |
...
| H70 | **Coord / matching SETS have no representation — one purchase, N slots, N fragmented items.** The app models single garments in single slots; a garment bought and conceived as a set (a suit set = blazer + skirt; a two-piece coord) fragments into per-garment items, each named lossily. A human names the whole set as one thing, so the SET NAME leaks onto a single piece: Zhiyun's "suit dress" was the **skirt half** of a blazer+skirt suit set, and that leaked name is what drove the mis-slot (`docs/plans/clothingtype-slot-correctness.md`). The "build a full outfit" logic also cannot know that two owned items form a coherent pre-matched pair. (Surfaced 2026-07-22, Zhiyun mis-slot case study.) | **OPEN — representation gap, M6-adjacent** | Adjacent to H25 (extensible representation) / H28 (pairwise edge seam) / H52 rung-3 (context-resolved multi-role). A set is naturally a learned/declared **edge** ("these go together"), NOT a new `clothingType` value or a new item type — do not try to solve it in the 5-type plumbing. **Not friend-blocking:** once each piece is slotted correctly (the clothingType-slot-correctness plan), the individual items work; the set-awareness is an enrichment. Decide the representation with the M6 edge graph. |
| H71 | **The weather dimension is effectively dead — every render resolves `weather="mild"` (22/22 live).** Two independent causes, both in the data (`weatherRaw` split): **(a) low geo adoption — 16/22 renders had NO lat/lon** (`weatherRaw==null`), so the resolver fell to `bucketFromSummary(occasion)` and the real occasions ("go traveling", "casual campus day") carry no weather keyword → default "mild" (`lib/mlRecommend.ts:161,175`); the geo prompt is browser-permission-gated and the admin-driver account has no browser at all (and since 2026-07-25 it is also **opt-in**: `dashboard/page.tsx` no longer calls `getCurrentPosition` on mount — see the resolution column). **(b) single-climate cohort — the 6 geo-granted renders hit the real open-meteo API** (`lib/weather.ts:129`), which returned "Clear sky, 22°C", and 22°C legitimately buckets to mild (`bucketFromTemp:188`, band 10–24°C). So the code is NOT broken (the geo→API path works, proven live); the dimension has ~zero variance because most renders lack geo AND a geographically-concentrated cohort has near-constant real weather. (2026-07-22 corpus mine.) **Sub-note:** `warmth` is the same invisible keyword-derived field (cardigan→10, blazer→8) but is **non-gating** — it only shifts the `option_path` compatibility bucket (`response.py:325-358`, "bucket never gates"), never yield — so it is this hole's degraded-dimension sub-note, not its own fix. | **OPEN — load-bearing for M6 corpus + out-of-region friends** | The M6 training corpus carries `weather` as a **constant** (a dead feature), and a friend in a genuinely hot/cold place gets climate-blind outfits (the prompt always says "Weather: mild", `rescue.py`). Orthogonal to the clothingType mis-slot (`docs/plans/clothingtype-slot-correctness.md`) — its own design call, Fable-reviewed when picked up: assertive geo prompt / manual temperature input / a stored home-location on the user. Do NOT bolt onto the mis-slot plan. Registered, not built. **2026-07-25 update — the "assertive geo prompt" lever is CLOSED by decision, not available:** the friend-first-use pass made geolocation opt-in (an in-app explanation + a tap, plus a Permissions-API resume so an existing grant is never re-asked), because a cold OS location dialog immediately after sign-in is a documented bail trigger. The corpus cost is near-zero and was weighed: adoption was never the binding cause — the 6 geo-granted renders returned 22°C→`mild` anyway, so cause (b), the single-climate cohort, is what actually flattens the dimension. **Do not "fix" this hole by restoring an automatic prompt.** The two remaining levers — manual temperature input, or a stored home-location on the user — are the ones that restore real variance, are climate-agnostic, and need no browser permission; they are what an out-of-region friend (flagged above) actually needs. |
...
| H100 | **Regenerate locks did not survive a re-roll** (friend-reported 2026-07-27 as "regenerate is not locking the item that was locked in the original prompt"): `RegenerateModal` is conditionally mounted and seeded `locked`/`disliked` from empty sets on every open. **Fixed — locks are now sticky across the re-roll lineage, client-remembered:** the dashboard remembers the `lockedItemIds` last submitted on an accepted re-roll (`activeLocksRef`, `app/(app)/dashboard/page.tsx` — set in `submitRegenerate`'s accept path and on a resumed re-roll's envelope, persisted in the sessionStorage dashboard blob for reloads) and seeds the next modal with them, **intersected with the displayed outfit's items** (the modal only renders toggles for what is on screen; a lock for an absent item would be invisible, un-clearable state — locks are a hard constraint per `fitted_core/ranker.py:362`, so the intersection normally drops nothing). A fresh root render resets the lineage (the §C.3 root-controls invariant is untouched and test-asserted). **Avoids are deliberately one-shot:** the engine excludes an avoided item from every child candidate, so an inherited avoid could never be displayed or cleared in the modal, and silent accumulation would starve a small closet (a live friend closet has 6 items) into "nothing buildable" with no visible cause; the durable "stop showing me this" signal is the Dislike feedback, not the per-roll steering control. Behavioral suite: `tests/dashboardRegenControls.test.tsx` (real component, mutation-checked RED). **Trap-guards that remain true:** (1) do not "fix" the rescue *anchor* — `lib/mlRecommend.ts:332` derives `forcedItemId` from the parent, never the client, verified on live data (Marc's re-roll kept the forced athletic short in all six children); (2) the register's old suggestion "seed the modal from the parent snapshot's stored `controls`" is **not client-reachable** — the row stores `controls` but the §A/G15 browser projection (`lib/mlSnapshotMerge.ts projectBrowserResponse`) never returns them; the client can only remember what it itself submitted, which is what the fix does. Watch-item (n=1, explained, not chased): a live child `c0` was item-identical to the parent's `c0` — consistent with regenerate being constrained *fresh* generation with no dedup against the parent's combos (`parentCandidateCacheKey` is only a payload cross-check), so a small closet under the same lens can reproduce a combo by chance. | **IMPLEMENTED 2026-08-23** — `app/(app)/dashboard/page.tsx` (`activeLocksRef` + `RegenerateModal initialLockedItemIds`), tests `tests/dashboardRegenControls.test.tsx`. | — |
| H101 | **The lens is dropped at the canonical-key layer, so every behavioural edge collapses across occasions.** §11 — the section that houses the ambition — specs the StyleEdge "between two items, **under a lens**". §7 then keys outfits lens-free (`BaseKey = topId:bottomId`, `FullSignature = BaseKey\|outer=…\|shoes=…`) and names those same keys as what drives "dislike cooldown … comboBoost/**edge matching**". The keys ARE the edge identity in the shipped code, so a 👎 recorded for "I am going to the gym" applies its cooldown to "formal interview" too, and a 👍 boosts the combination everywhere. §14's scoring formula (`baseScore + comboBoost + itemBoost − dislikePenalty`) has no occasion term either. **The wiring is not missing — only the consumption is:** `RankerContext` carries a *required* `occasion` field, plumbed from `buildLens` (`fitted/lib/mlRequestAdapter.ts:321`) through the service, and `fitted_core/ranker.py` reads it **exactly once**, at `:737`, to seed the **tie-break RNG** — determinism, never relevance. Friend-reported 2026-07-27 (both testers independently asked whether feedback is prompt-scoped); live corroboration: Zhiyun's 22 dislikes against `COOLDOWN_BUFFER_SIZE=10` on a closet with one pair of shoes. Harm is bounded, not absolute — cooldown drops go to the relax reserve, re-admittable at the C5 `cooldown_relaxed` rung (`ranker.py` `_apply_step4_filters`), so it shifts preference rather than removing availability. Distinct from **H28**, which reserves lens-conditioning for the *content* scorer: nothing currently covers the **behavioural** side. | **OPEN — the ambition's own dimension, missing at its foundation** | This is the highest-value row in the 2026-07-27 batch: the live corpus is already accumulating lens-free edges (45 interactions and growing), and the keys are persisted verbatim by M4, so the shape chosen here is what M6 inherits. Forward-compat door already open in §7: **H30** permits *appending* components to FullSignature in fixed canonical order without migration — but BaseKey is explicitly "base-only for `[NOW]`", and BaseKey is what the cooldown uses, so a lens-scoped cooldown is a BaseKey redefinition, not an append. Design call, Fable-review when picked up; decide it **with** H28 (one lens-conditioning story for both edge fields, not two). Do **not** resolve it by weakening §11 to match the code. |
| H102 | **The §16 structured "why" channel yields zero in practice.** Across 23 friend rejections on the live corpus, **0** carry `feedbackReason.codes`, `feedbackReason.rawText`, or `perItemFeedback`; the only populated row in the whole database belongs to Brian's own test account. This is working as designed — a one-tap dislike posts a reasonless `rejected` immediately and the "tell us why?" modal is optional (`fitted/lib/useDislikeEnrich.ts`, the D-3 durable-enrich contract) — but the consequence is that §23-**H34**'s "`rawText` stays provenance until reviewed/compiled" has nothing to compile, and Zhiyun's 22 dislikes on well-formed renders carry no diagnosis of *why* she rejected them. (2026-07-27 live-corpus read.) | **OPEN — designed-not-demonstrated, informational** | Not a bug and not friend-blocking; the primary read (accepted-vs-rejected) does not need reasons. Register so the §16 vocabulary is not mistaken for a populated training signal at M6, and so any future decision to make the reason prompt more assertive is weighed against the one-tap symmetry it would break (a dislike currently costs the same tap as a like — that symmetry is why the dislike arm has data at all). |
| H103 | **The live image corpus carries no EXIF at all, so the preregistration's mandatory `exif_transpose` is a guaranteed no-op.** All **38** stored images have zero EXIF entries — and decisively, **32 of 38 are under `SKIP_BELOW_BYTES` (400 KB)**, the threshold at which `prepareImageForUpload` returns the file *untouched* (`fitted/app/(app)/wardrobe/page.tsx:1439`). The client pipeline therefore never ran on them: the orientation data was already absent **at pick time**. Sizes cluster at 90–150 KB, consistent with sources that have been through a messaging app or gallery re-save before upload. Consequences: **(a)** prereg §6's stated rationale — "the Track-2 corpus is orientation-mixed, and un-transposed phone photos roughly halve the signal" — does not describe this corpus, and its required orientation-6 regression fixture exercises a path real data cannot take; **(b)** 8/38 are landscape and at least two (Marc's grey baggy jeans, black Levi jeans) present the garment rotated ~90°, now **permanent** — no tag remains for anything downstream to correct; **(c)** embedding fidelity is set by whatever the intermediary app compressed to, not by the client's 1280 px / q0.85 pipeline. Sub-note, same remedy — **frame composition**: several of Marc's photos include his feet and room clutter and two jacket shots contain a second garment; one of Zhiyun's 15 is not a camera capture at all but a **cropped screenshot** — 1170×1458 against her other fourteen at 1280×1707, and 1170 px is the native screen width of an iPhone 12/13/14-class device. It shows the garment worn, cropped below the face, outdoors. **Whether the wearer is her or a listing model is NOT determinable from the file, and the distinction does not matter here** — either way it is a re-encoded screen grab rather than a photograph of the garment, which is exactly the class the onboarding line now warns against. **Severity ordering, because "looks unusual" is not the same as "measured to hurt" —** the frozen preprocess is `Resize(224) → ConvertMode → ToTensor → Normalize` with **no crop step** (`embedding_manifest_fashionsiglip.json`), so the whole frame is embedded for *every* photo in the corpus, not just the odd ones. Ranked by evidence: **(1) rotation is the only image property H26 actually measured, and it is large** — closet-probe AUC 0.4375 → 0.5625 after `exif_transpose` (results.md §6); **(2) a second garment in frame** corrupts what the item *is* rather than where it is (two of Marc's outerwear shots contain navy trousers, so the "Overcoat" vector encodes trousers) — label noise, and the more damaging of the two composition issues; **(3) on-model / outdoors / not-on-a-bed is NOT established as harmful and plausibly helps** — Marqo-FashionSigLIP is trained on fashion e-commerce imagery, which is dominated by on-model product photography, so a daylight worn shot may sit closer to the backbone's training distribution than a night-time flat-lay on a duvet; **(4) background clutter generally** is unknown but roughly uniform across all 38, so it largely cancels. Do not spend onboarding goodwill on (3) or (4). **Aspect ratio is the cheap detector** for the screenshot class: anything off the camera's native 3:4 deserves a look. (2026-07-27 corpus read, `PIL.Image.getexif` + dimensions over the M6 export.) | **OPEN — onboarding-fixable, not code-fixable** | The client is behaving correctly; no code change reaches this. The lever is the onboarding message (runbook §8): ask for uploads straight from the camera roll, not routed through a messaging app or screenshotted, and one garment per frame on a plain surface. Applied 2026-07-27. Do **not** amend the frozen preregistration — this is a finding *about* the corpus that §6's mitigation does not cover, and the freeze is a one-way door. |
| H104 | **The export certificate can exclude only ONE operator account, so a second personal account counts as a friend.** `resolveExcludedUsers` (`fitted/scripts/exportTrack2Core.cjs:227`) accepts a single `operatorAuthId` plus the `track2test_*` regex. Brian's second personal account `brian.li.fmlbkup@gmail.com` (`authId HftfgRJg7fXeXhmZxrSNgdvaFei1`) matches neither, so it lands in the headline pool: today's certificate reports `"friends": 3` when only two are real testers, and contributes 1 accepted / 2 rejected scoreable clusters to a read whose decision floors are 25 and 25. Prereg §5 author-exclusion integrity. Distinct from **H97**, which covers an *unresolvable* operator id rather than a second valid one. (2026-07-27 export read.) | **OPEN — freeze-integrity, pre-Look** | Accept a **list** of operator auth ids (`--operatorAuthId` repeatable, or a comma list), keeping the single-value form working; report each exclusion by reason in `yield.excluded` as it already does. Cheap and pre-Look, so it can land before the certificate is ever read for a decision — but it is a **scripts/** change, so it is registered here rather than made during the read-only campaign. |
| H105 | **Three of §12's four precedence tiers do not exist in the shipped ranker.** §12 states the precedence "hard constraints (dress code / weather / comfort) > prompt occasion & formality > active StyleProfile shapes choices *within* the valid context > revealed negative signal suppresses bad repeats". Audited tier by tier against code: **(1) hard constraints** — `RankerContext.weather` is read once, at `ranker.py:737`, the tie-break RNG seed; weather's only real effects are a line in the LLM prompt and a post-rank display bucket, never a constraint or a filter. **(2) prompt occasion & formality** — occasion → prompt + the same RNG seed (H101); **`formality` is not in the data model at all**, scope-trimmed at M4 alongside `material`/`styleTags`. **(3) active StyleProfile** — exists only as three dormant columns on `fitted/models/GenerationSnapshot.ts:280-286` (`styleProfileId`, `styleProfileVersion`, `styleProfileSnapshot`); no model, no writer, no reader anywhere in `fitted_core` or `fitted/`. **(4) revealed negative signal** — the only implemented tier, and lens-blind per H101. Net consequence: **the LLM prompt is the sole mechanism making an outfit occasion-appropriate, and nothing downstream verifies it** — the deterministic Python layer that §5 insists must own ranking is, on the context dimension, a pass-through. Scope-check worth keeping: every `UPPER_SNAKE` constant in `fitted_core/config.py` was checked for defined-but-unused and came back **clean**, so this is specifically the lens/context dimension, not general dead code. (2026-07-27, prompted by friend feedback.) | **OPEN — ambition-fidelity, the §12 prose is not current behaviour** | §12's precedence sentence now carries a pointer to this row; it is a statement of the **target**, not of shipped behaviour, and must not be read as a contract. Resolution is not one fix — it is the sequencing question of which tiers M6 restores. **H71** already owns the weather half (its remaining levers: manual temperature input or a stored home location). Sub-note to fold in when H71 is next touched: H71's main text cites "the ranker's `WEATHER_WARMTH_BAND` penalty", which does not exist — the constant is consumed in `fitted_core/response.py:208-209` for the option-path display bucket; H71's own sub-note states this correctly while its main text does not. |
...
returned 12/12, finish `stop` — validated record in `service/config.py`; re-check driver = the
`tokcap-full-ask` gauntlet persona, runbook §8). The **rescue** worst-case ask (unbounded by the
daily-12 cap — `_rescue_candidate_requested` clamps to [6, 40]) was **not** re-validated live and is a
graded residual (TOKCAP-2, runbook §8); it degrades gracefully (truncation → repair → "couldn't find
