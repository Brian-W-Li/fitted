...
- **Update-path skips schema validators:** wardrobe PATCH uses `findOneAndUpdate(..., {new:true})` **without
  `runValidators:true`** (`wardrobe/[id]/route.ts:114`), so enum/schema constraints are bypassed on edits
  (raw IDs also flow straight into Mongoose → `CastError`→500 instead of a 400; the systemic ObjectId-validation
  gap). M5 validates through the §15 request adapter (R12).
...
These are legacy-frontend, deferred to the M5 cutover (deletion license: `fitted/` code is rewritten, not
patched piecemeal). Registered here so the M5 UI rewrite closes them; **not** actioned now (no real users).
