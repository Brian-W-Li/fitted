...
floor + the daily ask ceiling are re-tuned together, and the worst-case capped-ask validation is
**done** (TOKCAP-1 discharged 2026-07-20: full 12-outfit ask under the 2200 cap returned 12/12,
finish `stop` — validated record in `service/config.py`; re-check driver = the `tokcap-full-ask`
gauntlet persona, runbook §8). *(The v1.2
