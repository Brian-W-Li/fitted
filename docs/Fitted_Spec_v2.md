...
| **M6 (the dive)** | Trained edge/graph scorer — the **content-compatibility prior lands on the §23-H28 additive pairwise/outfit-level `rank()` hook** (the non-transitive, type-conditioned shape the item-level `SignalScorer` sampler slot cannot represent), **gated by the H26 spike above — which returned NO-GO by the frozen letter (a gate-B power miss at the frozen cap, not an accuracy miss), so M6 does not open on H26's authority alone**: entry requires re-powering gate B (extend the judged prefix over the frozen `fitb_order.json`) **and re-measuring the catalog→closet transfer on real-ingestion data** — H26 *reports* that transfer, it does not gate on the underpowered single closet — build doc §12/§13. **Re-measure prereg (2026-07-20 merit audit, Fable-decided): the decision rule must be written + frozen BEFORE the first friend's data is exportable.** The inherited H26 healthy band is structurally undecidable at cohort N≈30–60 accepted outfits (Hanley-McNeil half-widths ±0.09–0.13: the `CI_low(AUC_closet) ≥ 0.70` floor read needs a point estimate *above the catalog AUC itself* (0.7315) — unpassable; the `CI_high(drop) ≤ 0.12` read fails even at true drop 0 half the time). The prereg replaces it with (i) a **two-boundary directional rule** — report which of {above-chance: CI_low > 0.50, below-healthy: CI_high < 0.70} is excluded; (ii) **primary read = accepted-vs-rejected discrimination** within-user (human-judged negatives — fixes H26 §9.9's mechanical-negative proxy; ~doubles effective data per friend; decidable 0.5 boundary at achievable N); (iii) the `exportTrack2Core.cjs` DECIDABLE verdict hardened from a raw ≥30 count to a scoreable-cluster certificate (≥2-item outfits, same-category-negative availability, per-friend spread). The prereg session re-derives the exact N′ and boundaries itself (freeze-before-look — the discipline that made the H26 NO-GO credible); offline NDCG@k / AUC / FITB on the universal content prior (zero-user, runnable); the **behavioral/personalization** signal (which *does* ride the item-level `SignalScorer` slot) → learned + online A/B is the **user-dependent arm** (`[STAGED]`/`[NORTH-STAR]` on a no-users fork — §11/§23-H9) | `[STAGED]` |
