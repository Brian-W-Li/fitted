...
**Sequence from here (set 2026-06-27, post-M4 consolidation; amended by M5 cutover review 2026-07-06).** The order is **consolidation → H26 spike ✅ (done 2026-07-05; NO-GO by the frozen letter — see the §20 H26 row + `ml-system/experiments/h26/results.md`) → M5**. M5 lands the producer-side scorer trace seam only; the additive rank-order hook is deferred to M6 entry so cold-start compatibility does not reorder shipped behavior. M5 is now the immediate next rung. The **M5 deletion license is fully open** (no real users; this is a demonstrable-concept fork) — the legacy recommend/regenerate vertical and the §19 dresses string-match arm are **cut, not migrated**, when `USE_ML_SHORTLISTER` flips. Post-M5, the **W-track** (ingestion surface) and **B-track** (text boards → StyleProfile) are **coequal** — neither blocks the other, and the now-explicit someday-launch path (rejoin the CS-148 team *or* post + advertise to friends) makes the B-track user-facing surface a real downstream destination, not a dead end.
