...
| 7 | **Response + StyleMove** | Outfits[] + backend-assigned `optionPath`/`risk` + StyleMove + scoreBreakdown; write GenerationSnapshot; async log | Spearhead labels · M5 snapshot |
...
boundary), and M5 adds Step 7's snapshot/logging plus the service deploy (no candidate cache — D2).
...
The substrate (`ml-system/fitted_core/`, Python, pytest, no DB/keys) has M0–M3, the Spearhead rescue vertical, M4 (data path + dormant snapshot substrate), and the H26 spike complete; M5 (the live cutover) is mid-build (C1–C3 landed). Per-row status below.
...
| H7 | `generationIndex` lifecycle (ownership, range, increment, reset) was undefined — it is the lineage/index field distinguishing a re-roll | **RESOLVED-DESIGN (M5 §C.1)** → implement at C4/C5 | M5 defines server ownership: first render index 0; regenerate child index = parent+1 from an ownership-verified parent re-read; client-supplied child index is ignored; duplicate `requestId` identity excludes clock-derived `seedDate` to avoid false conflicts. Load-bearing for re-roll lineage/idempotency (`m5-cutover.md` §C.1/§C.4 — no cache). |
| H8 | Daily-reseed `date` timezone (server-UTC vs user-local) was undefined | **RESOLVED-DESIGN (M5 §F)** → implement at C5 | M5 fixes `seedDate` as UTC `YYYY-MM-DD`, computed by the Next adapter and passed to the service; the service does not read a clock. Must be identical across the adapter and service payload or seed/key provenance drifts at the day boundary. |
...
| H13 | Cross-runtime CI / runtime reproducibility (workflow + pins + conformance) | **RESOLVED-DESIGN (M5 plan)** → C8 pre-flip gate | M5 pins the service runtime/dependencies and requires Python+TS conformance green **before `USE_ML_SHORTLISTER` flips**; it is not an M5 entry gate and not a reason to block C4. The workflow file can land with the C8 cleanup commit, but the tests must be green as a commit-1 pre-flip gate (`m5-cutover.md` Verification/§J). |
