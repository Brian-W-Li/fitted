/**
 * Legacy recommend vertical (M5 C5, seam #6) — the pre-M5 direct-OpenAI shortlist→LLM→validate
 * daily recommender, extracted VERBATIM from the old `route.ts` so the flag-off arm keeps the
 * deployed behavior unchanged. `route.ts` calls `legacyRecommend` when `USE_ML_SHORTLISTER` is off.
 *
 * C8 commit-2 deletes this whole file + its one-line call site (the plan's §19 delete list) — the
 * live route.ts is never surgically excised.
 *
 * The OpenAI client is constructed LAZILY inside the handler (not at module scope): once
 * `OPENAI_API_KEY` moves to the Fly service at C8, importing this module must not throw, so the
 * M5 arm's `import` of the shared route file stays safe.
 */
...
export async function legacyRecommend(request: NextRequest) {
