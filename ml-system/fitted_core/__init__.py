"""Fitted v1.2 recommendation substrate (M0/M1).
...
Error-model convention (applies across the package):
  - **Expected, data-driven failures return an error channel** — a
    ...
    §6.3 rules. ``normalize_to_slotmap`` / ``is_valid_slotmap`` use this form.
  - **Precondition / caller-contract violations raise ``ValueError``.** These mean
    ...
    (``keys``, ``template_of``), a sampler handed an over-cap list with no sampler
    (``apply_cap``), a wire value the internal dataclass guards reject
