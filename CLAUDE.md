...
5. M5: **✅ done** (C1–C8; cloud deploy live 2026-07-16, `docs/plans/m5-cutover.md` + runbook §8). Deployed the Python render service (Fly, single-machine pin); wired the Next app behind `USE_ML_SHORTLISTER`; live GenerationSnapshot write + the runtime authenticity gate; full recommend/regenerate rewrite (regenerate = constrained fresh generation, no cache). Track 2 hardened + pushed the deployment for friend data collection.
