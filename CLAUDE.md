...
Goal: build the v2 recommendation substrate first, then use it as the seam for the trained style-graph scorer. The substrate (`ml-system/fitted_core/` M0–M3 + the Spearhead rescue vertical) and the M4 data/snapshot layer are **built**. The **H26 compatibility spike** is **in build** (offline; `docs/plans/h26-compatibility-spike-v2.md`; the zero-user demonstrable ML result) — **C1–C4 code committed** (data layer; frozen pre-registration; baselines + both trained heads + the eval-driver metric half + the gate-B `fitb_order.json`; the `gpt-5.4-mini` judge + gated emission + RUN tooling + the §F panel calibration); **B2 done** (`selection.json` sealed + committed off the built 83,178-id embedding cache); **now: the C4 RUN phase** (panel calibration → judge pilot → blind addendum freeze → gate-b → emit). Then the **M5 service cutover** (deploy + `USE_ML_SHORTLISTER` + live snapshot write). Sequence: consolidation → H26 → M5 (Spec §20).
...
3. M4: **✅ done** (C1–C8, `docs/plans/m4-data-model-migration.md` §14). **M4a** (data path — DB wipe, 5-value `clothingType` + keyword-derived `warmth`, PreferenceSummary rip, ingestion rebuild; C1–C3) committed + live-verified. **M4b** (dormant snapshot substrate — fitted_core version constants + `snapshot_serde` wire layer, the GenerationSnapshot model + immutability guard + indexes, the Python `GenerationSnapshotPayload` + Option-B trace siblings, the `wardrobeimages` cascade, the e2e contract fixture; C4–C8) committed + heavy-audited, ships dormant. `material`/`formality`/`styleTags` columns + cascade-redaction wiring scope-trimmed to later milestones. **M5 owns the live cutover — see the plan's §14.5 M5-handoff note.** Current suite floor: **≥715 pytest green / ≥366 jest** (floors grow, never pins).
4. **H26 compatibility spike (offline, IN BUILD — C1–C4 code committed (data layer, C2 prereg freeze, baselines + trained heads + eval driver, `gpt-5.4-mini` judge + gated emission + RUN tooling); B2 `selection.json` sealed + committed; now the C4 RUN phase (panel calibration → pilot → blind freeze → gate-b → emit); `docs/plans/h26-compatibility-spike-v2.md`):** a public-corpus content-compatibility baseline (Polyvore disjoint split; AUC + FITB vs a `gpt-5.4-mini`-as-judge baseline) — the zero-user demonstrable ML result + the go/no-go on the trained scorer; settles the H28 seam shape (pairwise/edge) before M5 wires it. Not gated behind M5; it is the **current rung** (consolidation → H26 → M5), slotted before M5 wires the scorer call. The thesis is a **systems decision — _when does a tiny specialized model beat a per-edge LLM call?_ — not a quality contest** (`gpt-5.4-mini` is the production stylist — `recommend/route.ts`; a mini-tier model, so the bar is parity-not-superiority, and the §9 cost/determinism/availability table is the headline artifact); the **go/no-go is A∧B∧D** (the catalog→closet transfer is **reported, not gated** — too underpowered on one closet — and becomes an M6 re-measure entry condition); a no-go still ships as a clean engineering verdict. (Spec §20 + §23-H26/H28.)
...
6. M6: train the style-graph scorer — the content-compatibility prior lands on the §23-H28 pairwise/outfit-level `rank()` hook (not the item-level `SignalScorer` sampler slot, which stays the behavioral/personalization seam), gated by the spike; evaluate with GenerationSnapshots + feedback.
...
- `ml-system/fitted_core/`, `ml-system/README.md` — current substrate implementation. `docs/plans/m3-ranker.md` is the **completed M3 ranker reference** (C1–C6; per-checkpoint detail in its §11 checkpoint table); `docs/plans/m2-validator.md` is the completed M2 validator reference; `docs/plans/m0-m1-substrate.md` is completed M0/M1 context; `docs/plans/spearhead.md` is the **completed Spearhead reference** (orphan-item rescue, C1–C6; C6/H40 live-eval in its §E). `docs/plans/m4-data-model-migration.md` is the **completed M4 reference** (C1–C8; the §14.5 M5-handoff note records what M5 inherits/owns). **Next active work, in order: the pre-M6 H26 compatibility spike (in build — C1–C4 code committed, now the C4 RUN phase; `docs/plans/h26-compatibility-spike-v2.md`, §20) — the current rung — *then* the M5 cutover (live snapshot write behind `USE_ML_SHORTLISTER`, per §14.5). Sequence = consolidation → H26 → M5 (Spec §20). M4 (data path + dormant snapshot substrate) is done.**
